...
@section('body')
  <section class="content-header">
    <h1>
      {{ __('translations.edit') }} {{ __('translations.transfer_number') }} {{ __('translations.for_store') }} (( {{ $order->store->name }} ))
    </h1>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-primary" style="padding-right: 10px;">
                  <!-- form start -->
                  {!! Form::open(['route' => 'transfers.edit.transfer_id.save', 'files' => true, 'enctype' =>'multipart/form-data']) !!}

              <input class="form-control" value="{{$order->id}}" type="hidden" name="ship_id">
              <div class="form-group{{$errors->has('transfer_id') ? 'error' : null}}">
                  <label for="Store"> * {{__('translations.transfer_number')}}</label>
                  <input type="number" class="form-control" id="" name="transfer_id" value="{{$order->transfer_id}}">
              </div>
