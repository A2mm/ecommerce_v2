...
@section('body')
  <section class="content-header">
    <h1>
      {{ __('translations.edit_store_to') }}  (( {{ $order->store->name }} ))
    </h1>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-primary" style="padding-right: 10px;">
                  <!-- form start -->
                  {!! Form::open(['route' => 'transfers.edit.store_id.save', 'files' => true, 'enctype' =>'multipart/form-data']) !!}

              <input class="form-control" value="{{$order->id}}" type="hidden" name="ship_id">

                <div class="form-group{{$errors->has('store_id') ? 'error' : null}}">
                  <label for="Store"> * {{__('translations.choose_store')}}</label>
                
                  <select class="form-control" id="" name="store_id">';
                      @foreach($stores as $store)
                        <option value="{{$store->id}}" {{$order->to_store == $store->id ? 'selected' : ''}}> 
                          {{ $store->name }}
                        </option>
                      @endforeach 
                    </select> 
