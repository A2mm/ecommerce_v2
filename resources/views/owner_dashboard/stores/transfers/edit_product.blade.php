...
@section('body')
  <section class="content-header">
    <h1>
      {{ __('translations.edit') }}  {{ __('translations.product') }} {{ __('translations.for_store') }} (( {{ $order->store->name }} ))

       <h3 style="color: blue;"> {{ __('translations.product') }} : {{ $order->product->name }}</h3>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-primary" style="padding-right: 10px;">
                  <!-- form start -->
                  {!! Form::open(['route' => 'transfers.edit.product_id.save', 'files' => true, 'enctype' =>'multipart/form-data']) !!}

              <input class="form-control" value="{{$order->id}}" type="hidden" name="ship_id">               
              <div class="form-group{{$errors->has('product_id') ? 'error' : null}}">
                  <label for="Store"> * {{__('translations.choose_product')}}</label>
                  <select class="form-control" id="" name="product_id">
                     <option selected="selected" disabled="disabled" value=""> {{ __('translations.choose_product') }}</option>
