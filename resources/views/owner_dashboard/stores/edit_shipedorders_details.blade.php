...
@section('body')
  <section class="content-header">
    <h1>
      {{ __('translations.edit_shiporder') }} {{ __('translations.for_store') }} (( {{ $order->store->name }} ))
    </h1>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-primary" style="padding-right: 10px;">
                  <!-- form start -->
                  {!! Form::open(['route' => 'edit.shipedorders.details.store', 'files' => true, 'enctype' =>'multipart/form-data']) !!}
              ...
              <input class="form-control" value="{{$order->store_id}}" type="hidden" name="store_id">
                ...
                <div class="form-group{{$errors->has('store_id') ? 'error' : null}}">
                  ...
                  <input disabled="disabled" type="text" class="form-control" id="" name="store_name" value="{{$order->store->name}}">
                  <?php /*
                  <select class="form-control" id="" name="store_id">';
              ...
              <div class="form-group{{$errors->has('shiporder_id') ? 'error' : null}}">
                  <label for="Store"> * {{__('translations.shiporder_id')}}</label>
                  <input type="number" class="form-control" id="" name="shiporder_id" value="{{$order->shiporder_id}}">
              </div>

              <div class="form-group{{$errors->has('product_id') ? 'error' : null}}">
                  <label for="Store"> * {{__('translations.product')}}</label>
              ...
              <div class="form-group{{$errors->has('quantity') ? 'error' : null}}">
                  <label for="Store"> * {{__('translations.quantity')}}</label>
