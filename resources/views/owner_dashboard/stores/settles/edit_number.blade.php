...
@section('body')
  <section class="content-header">
    <h1>
      {{ 'تعديل رقم التسوية ' }} {{ __('translations.for_store') }} (( {{ $order->store->name }} ))
    </h1>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-primary" style="padding-right: 10px;">
                  <!-- form start -->
                  {!! Form::open(['route' => 'settles.edit.settle_id.save', 'files' => true, 'enctype' =>'multipart/form-data']) !!}

              <input class="form-control" value="{{$order->id}}" type="hidden" name="ship_id">
              <div class="form-group{{$errors->has('settle_id') ? 'error' : null}}">
                  <label for="Store"> * {{__('translations.settle_number')}}</label>
                  <input type="number" class="form-control" id="" name="settle_id" value="{{$order->settle_id}}">
              </div>
