...
.zoom:hover {
  ...
  <section class="content-header">
    ...
    {{__('translations.stores_settlements')}}
  ...
  <div class="product_movement" style="padding: 25px; border-radius: 25px; width: 600px;">
      ...
      <form class="form" method="get" action="{{route('manage.allstores.settlements')}}">
...
<a style="height: 33px; margin-right:10px;" class="btn btn-sm btn-success" href="{{route('storesSettles.excel', ['from' => $from, 'to' => $to]) }}">
  ...
  <!-- Main content -->
                  ...
                  @if($user->can('edit settles details') || $user->can('Administer'))
                    <a href="{{ route('edit.settles.details', ['id' => $history->id])}}" ><button class="btn btn-primary btn-sm"><i class="fa fa-edit" aria-hidden="true"></i> {{__('translations.edit')}} </button></a>
                  @endif 

                  @if($user->can('add new stock settle') || $user->can('Administer'))
                    <a href="{{ route('stock.add.settle', ['id' => $history->id])}}" ><button class="btn btn-success btn-sm"><i class="fa fa-plus" aria-hidden="true"></i> {{__('translations.add')}} </button></a>
                  @endif 
                </td>
                      </tr>
                      @endforeach
             ...
             {!! $storesSettlements->appends(["from" => $from, "to" => $to])->render() !!} 
                  </div>
