          ...
          <thead>
            ...
            {{-- <tr>
              ...
              <th>
                <table class="table table-bordered table-striped">
                  <tbody>
                    @foreach ($purchase->stores() as $key => $value)
                      <tr>
                        <th>{{$value->name}}</th>
                      </tr>
                    @endforeach
        ...
        <table class="table table-bordered table-striped">
          ...
          @if($history->store_id == null)
            <tr>
              ...
              <th>
                <ul>
                  @foreach ($purchase->productStores($history->product->id) as $store)
                    @if (abs($purchase->storeQuantities($store->id , $history->product->id)) > 0)
                      <li>{{$store->name}} ({{abs($purchase->storeQuantities($store->id , $history->product->id))}})</li>
                    @endif
                  @endforeach
                </ul>
              </th>
