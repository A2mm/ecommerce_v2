...
<div class="col-md-4">
  ...
  <div class="form-group{{$errors->has('slug') ? 'error' : null}}">
                        <label for="name">* {{__('translations.slug')}}</label>
                        <input class="form-control" id="slug" value="{{old('slug')}}" type="text" name="slug">
                        @if($errors->has('slug'))
...
<div class="form-group {{$errors->has('seo_description') ? 'error' : null}}" style="width:80%">
  ...
  <textarea  rows="5" name="seo_description" class="form-control" id="seo_description">{{old('seo_description')}}</textarea>
...
<div class="form-group {{$errors->has('product_benefits') ? 'error' : null}}" style="width:80%">
  ...
  <textarea id="mycontent2" rows="5" name="product_benefits" class="form-control" id="product_benefits">{{old('product_benefits')}}</textarea>
...
<div class="row">
<div class="col-md-12">
  <div id="store_tag" style="margin-top:20px;"></div>
</div>
...
<div class="form-group{{$errors->has('subcategory_id') ? 'error' : null}}" style="width:40%;display: inline-block; margin: 0px 30px;" id="subcategory_id_div">
                       ...
                       <div class="form-group" style="width:20%; margin: 10px 44px;">
                        <label for="category_online_id">فئة اونلاين</label>
                        <select name="category_online_id" class="form-control" id="category_online_id">
                          <option disabled selected>{{'اختر فئة'}}</option>
                        @foreach($categoriesOnline as $category)
                        <option value="{{$category->id}}" >{{$category->name}}</option>
                        @endforeach
                        @if($errors->has('category_online_id'))
                          <span class="help-block" style="color:red;">{{$errors->first('category_online_id')}}</span>
                          @endif
                        </select>
                      </div>
              ...
              <div class="col-md-6">
                <div class="form-group{{$errors->has('otherprices.'.$i) ? 'error' : null}}">
                  <label for="store">* {{__('translations.price')}}</label><br>
                   @if($i == 0)
                  <input type="text"  class="form-control" name="otherprices[]" value="{{old('otherprices.'.$i)}}" min="0">
                  @else
                    <input type="text"  class="form-control" name="otherprices[]" value="{{old('otherprices.'.$i)}}" min="1">
                  @endif
  ...
  var old_stores = [];
    ...
    $('#add_store').on('click',function(){
      ...
      <div class='form-group'>
        ...
        <input class="form-control" type="number" step="1" min='1' max='999999' required name="store_quantities[quantity][]" placeholder="Quantity" onKeyDown="if(this.value.length==8 && event.keyCode!=8) return false;">
